...
export default function GraficoActividadStreams({
  ...
  useEffect(() => {
    ...
    setLoading(true)
    const promise = fetch(`${API}/atletas/${atletaId}/activity_streams?sesion_id=${sesionId}`)
      .then(r => r.json())
      .then(r => {
  ...
  // usarLaps = modo laps en el gráfico
  // Para run/swim: laps SIEMPRE, aunque lleguen streams (evita parpadeo)
  // Para bike: streams si llegaron, sino laps
  const usarLaps = tieneLaps && laps.length >= 2 && (sport !== 'cycling' || !tieneStreams)
  ...
  const sportEmoji = sport === 'cycling' ? '\u{1F6B4}' : sport === 'swimming' ? '\u{1F3CA}' : '\u{1F3C3}'
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 22 }}>{sportEmoji}</span>
          {[
            act?.duration_min && { icon: '⏱', val: fmtDur(act.duration_min), color: D.text2 },
            distKm            && { icon: '📏', val: fmtDist(distKm),          color: D.text2 },
            (act?.hr_avg||stats.hr_avg) && { icon: '❤️', val: `${Math.round(act?.hr_avg||stats.hr_avg)} bpm`, color: D.hr.line },
            (act?.hr_max||stats.hr_max) && { icon: '💓', val: `máx ${Math.round(act?.hr_max||stats.hr_max)}`, color: '#F97316' },
            (act?.np_watts||stats.power_np) && { icon: '⚡', val: `${act?.np_watts||stats.power_np}W NP`, color: D.power.line },
            (act?.potencia_media||stats.power_avg) && { icon: '📊', val: `${Math.round(act?.potencia_media||stats.power_avg)}W avg`, color: '#84CC16' },
            act?.wkg       && { icon: '⚖️', val: `${act.wkg.toFixed(2)} w/kg`,      color: '#A78BFA' },
            act?.tss_total && { icon: '🎯', val: `TSS ${act.tss_total.toFixed(0)}`,  color: '#38BDF8' },
            ...
            act?.calorias  && { icon: '🔥', val: `${act.calorias} kcal`,             color: '#F97316' },
            (act?.pace&&sport==='running') && { icon: '🏃', val: fmtPace(act.pace)+'/km', color: sportColor },
        ...
        {hS && (
          <div style={{
            ...
            {hS.hr && (
              <span style={{ color:D.zone[getZone(hS.hr,actLthr)], fontWeight:700 }}>
                ❤️ {Math.round(hS.hr)} bpm
                <span style={{ color:D.text3, fontWeight:400, marginLeft:4, fontSize:9 }}>
                  {getZone(hS.hr,actLthr)}
                </span>
              </span>
            )}
            {hS.power   && <span style={{ color:D.power.dot, fontWeight:700 }}>⚡ {Math.round(hS.power)}W</span>}
            {hS.pace    && <span style={{ color:D.pace.dot,  fontWeight:700 }}>🏃 {fmtPace(hS.pace)}{paceUnit}</span>}
            {hS.cadence && <span style={{ color:D.cad.dot,   fontWeight:600 }}>🔄 {Math.round(hS.cadence)}</span>}
            {hS.alt     && <span style={{ color:D.text3,     fontSize:10 }}>⛰ {Math.round(hS.alt)}m</span>}
            {hS.dist_km != null && (
      ...
      {series.length >= 1 && CANALES.length > 1 && (
        <div style={{
          padding:'8px 20px 6px', display:'flex', gap:6, alignItems:'center',
          borderBottom:`1px solid ${D.border}`,
        }}>
          <span style={{ fontSize:9, color:D.text3, textTransform:'uppercase',
            letterSpacing:1, marginRight:4 }}>Canales</span>
          {CANALES.map(c => (
      ...
      {series.length >= 1 ? (
        <svg ref={svgRef} width={W} height={H} viewBox={`0 0 ${W} ${H}`}
          ...
          {canalesOn.pace && pacePath && (
            <path d={pacePath} fill="none" stroke={D.pace.line} strokeWidth="2"
              strokeDasharray="6,3" clipPath={`url(#clip_${sesionId})`}/>
          )}

          {/* Potencia línea */}
          {canalesOn.power && powPath && sport !== 'swimming' && (
            <path d={powPath} fill="none" stroke={D.power.line} strokeWidth="2"
              clipPath={`url(#clip_${sesionId})`}/>
          )}

          {/* HR línea */}
          {canalesOn.hr && hrPath && (
            <path d={hrPath} fill="none" stroke={D.hr.line} strokeWidth="2.5"
              clipPath={`url(#clip_${sesionId})`}/>
          )}
      ...
      {series.length >= 1 && (
        <div style={{
          padding:'8px 20px 12px', borderTop:`1px solid ${D.border}`,
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
            {[
              { icon:'⏱', label:'Duración',  v: act?.duration_min ? `${Math.round(act.duration_min)} min` : '--' },
              { icon:'📏', label:'Distancia', v: fmtDist(distKm) },
              act?.hr_avg    && { icon:'❤️', label:'FC avg',  v:`${Math.round(act.hr_avg)} bpm`,  c:'#EF4444' },
              act?.hr_max    && { icon:'💓', label:'FC máx',  v:`máx ${Math.round(act.hr_max)}`,  c:'#F97316' },
              act?.tss_total && { icon:'🎯', label:'TSS',     v:act.tss_total.toFixed(1),          c:'#38BDF8' },
              act?.np_watts  && { icon:'⚡', label:'NP',      v:`${act.np_watts}W`,                c:'#F59E0B' },
              act?.calorias  && { icon:'🔥', label:'kcal',    v:`${act.calorias}` },
            ].filter(Boolean).map((m,i) => (
              <div key={i} style={{
                flex:1, minWidth:80,
                ...
                <div style={{ fontSize:10, color:D.text3, textTransform:'uppercase',
                  letterSpacing:0.8, marginBottom:4 }}>{m.icon} {m.label}</div>
...
function TablaLaps({ laps, sport, lthr, hover, setHover, ftp }) {
  ...
  if (sport === 'cycling') {
    return (
      <div style={{ padding:'4px 20px 14px', overflowX:'auto' }}>
