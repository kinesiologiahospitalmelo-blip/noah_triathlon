...
viejo = """      {/* HEADER — imagen de fondo + glassmorphism flotante */}
      <div style={{
        ...
        <div style={{
          position: 'absolute', inset: 0,
          background: 'linear-gradient(90deg, rgba(5,5,15,0.88) 0%, rgba(5,5,15,0.45) 55%, rgba(5,5,15,0.78) 100%)',
        }}/>
        {/* Contenido flotante */}
        <div style={{
...
nuevo = """      {/* HEADER estilo Garmin — imagen hero grande (30%), nombre prominente,
          botones azules, logo arriba. Logica de sync identica. */}
      <div style={{ position:'relative', overflow:'hidden', minHeight:210 }}>
        {/* Imagen hero */}
        <div style={{
          position:'absolute', inset:0,
          backgroundImage:'url(/assets/hero_dashboard.png)',
        ...
        {/* Contenido */}
        <div style={{ position:'relative', zIndex:1, display:'flex', flexDirection:'column', minHeight:210 }}>
          {/* Logo + logout arriba */}
...
if viejo in contenido:
    ...
    print("ERROR: no matcheó — diferencias de CRLF/espacios")
    # Debug
    idx = contenido.find("minHeight: 90,")
    if idx >= 0:
        print(f"  'minHeight: 90,' encontrado en pos {idx}")
    idx2 = contenido.find("minHeight:90,")
    if idx2 >= 0:
        print(f"  'minHeight:90,' encontrado en pos {idx2}")
